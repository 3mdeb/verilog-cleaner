...
command -v verible-verilog-lint \
    ...
    | xargs curl -L | sudo tar xzf - -h --strip-components 1 -C /usr

reviewdog -reporter=${INPUT_REPORTER-local} -diff='git diff' -runners=${INPUT_RUNNERS} -conf=${ACTION_PATH-.}/.reviewdog.yml -tee
